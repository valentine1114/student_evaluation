...
// import Vue from "vue";
import { fetchStudentLeadership } from "@/services/studentLeadershipService";
import { teacherSubmitStudentLeadershipScore } from "@/services/studentLeadershipService";
import { fetchLeadershipScoresList } from "@/services/teacherListScoreService";
export default {
  props: ["msg"],
  watch: {
    msg(newVal) {
  ...
  methods: {
    async submitScore(data) {
      ...
      await teacherSubmitStudentLeadershipScore(this.score, data);

      // 提交评分的逻辑
      console.log("提交评分:", this.score);
      console.log("科研情况:", data);
      // this.$router.go({ path: "/Scoring/studentLeadershipList" });
    },
    async getDataList(msg) {
      console.log(msg);
      this.allDataList = await fetchStudentLeadership();
      console.log(this.allDataList);

      this.dataList = this.allDataList.filter(
        (item) => item.student.id === +msg
      );
