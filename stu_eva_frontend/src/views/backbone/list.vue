...
export default {
  ...
  methods: {
    ...
    getStatusButtonType(index) {
      ...
      if (item && item.leadershipScore != null) {
        return "success";
      }
      return "warning";
    },
  ...
  computed: {
    getStatusText() {
      return function (index) {
        ...
        console.log(item[0].leadershipScore != null);
        if (item[0].leadershipScore != null) {
          console.log(item[0].leadershipScore);
          return "已评测";
        }
        return "未评测";
      };
  ...
  async mounted() {
    ...
    fetchLeadershipScoresList().then((response) => {
      this.list = response;
