...
import { fetchResearchScoresList } from "@/services/teacherListScoreService";
...
export default {
  ...
  methods: {
    ...
    getStatusButtonType(index) {
      ...
      if (item && item.researchScore != null) {
        return "success";
      }
      return "warning";
    },
  ...
  computed: {
    getStatusText() {
      return function (index) {
        ...
        console.log(item[0].researchScore != null);
        if (item[0].researchScore != null) {
          console.log(item[0].researchScore);
          return "已评测";
        }
        return "未评测";
      };
  ...
  async mounted() {
    ...
    fetchResearchScoresList().then((response) => {
      this.list = response;
