        ...
        :key="pageInfo.currentPage"
          :data="
            ...
            dataList.slice(
              ...
              Math.min(pageInfo.currentPage * pageInfo.pageSize, dataList.length)
            )
          "
          border
          style="width: 100%"
...
import { fetchSocialScoresList } from "@/services/teacherListScoreService";
...
export default {
  ...
  methods: {
    ...
    getStatusButtonType(index) {
      ...
      if (item && item.socialPracticeScore != null) {
        return "success";
      }
      return "warning";
    },
    filterUnreviewed() {
      this.dataList = this.dataList.filter(item => item.socialPracticeScore == null);
    ...
    startReview(index) {
      ...
      // console.log("message"+this.massage);
      // this.massage = this.dataList[index].id;
      const dataIndex = (this.pageInfo.currentPage - 1) * this.pageInfo.pageSize + index;
  this.massage = this.dataList[dataIndex].id;
      console.log("data message"+this.dataList[message]);
  ...
  console.log('Current page has been changed to: ', val);
...
end = end > this.dataList.length ? this.dataList.length : end;
const currentPageData = this.dataList.slice(start, end);


  console.log('Data for current page: ', currentPageData);
},
    // 配置显示序号
    indexMethod(index) {
      return (
  ...
  computed: {
    getStatusText() {
      return function (index) {
        ...
        console.log(item[0].socialPracticeScore != null);
        if (item[0].socialPracticeScore != null) {
          console.log(item[0].socialPracticeScore);
          return "已评测";
        }
        return "未评测";
      };
  ...
  async mounted() {
    ...
    fetchSocialScoresList().then((response) => {
      this.list = response;
