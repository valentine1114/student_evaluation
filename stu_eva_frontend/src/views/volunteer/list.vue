...
export default {
  ...
  methods: {
    ...
    getStatusButtonType(index) {
      ...
      if (item && item.volunteerScore != null) {
        return "success";
      }
      return "warning";
    },
  ...
  computed: {
    getStatusText() {
      return function (index) {
        ...
        console.log(item[0].volunteerScore != null);
        if (item[0].volunteerScore != null) {
          console.log(item[0].volunteerScore);
          return "已评测";
        }
        return "未评测";
      };
  ...
  async mounted() {
    ...
    fetchVolunteerScoresList().then((response) => {
      this.list = response;
