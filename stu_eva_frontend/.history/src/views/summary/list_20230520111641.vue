...
import { fetchStudentScoresList } from "@services/teacherListScoreService.js";
...
export default {
  ...
  created() {
    console.log("created");
    teacherGetStudentScores().then((response) => {
      console.log(response);
      response.forEach((item) => {
        if (item.summaryScore !== null) {
          // 如果属性不为空，将按钮文字改为 "通过评测"，并改变颜色
          console.log(item.id);
        }
      });
    });
  ...
  computed: {
    getStatusText() {
      return function (index) {
        this.list = fetchStudentScoresList();
        fetchStudentScoresList().then((response) => {
          console.log(response);
          const item_id = this.indexMethod(index);
          const item = response.filter((item) => item.id === +item_id);
          console.log(item_id);
          console.log(item);
